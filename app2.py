...
# Plot actual prices
    plt.plot(scaler.inverse_transform(df1), label = "Closing Price")
    plt.plot(trainPredictPlot, label = "Training Predicted Price")
    plt.plot(testPredictPlot, label = "Testing Predicted Price")
    plt.xlabel("Date")
    ...
    # Use the last `time_step` data points from the original data to make predictions for the future
    future_test_data = df1[-time_step:].reshape((1, -1))

    for i in range(future_days):
        future_pred = model.predict(future_test_data.reshape((1, time_step, 1)))
        future_predictions.append(future_pred[0, 0])
        future_test_data = np.append(future_test_data[:, 1:], future_pred, axis=1)
